...
function capitalA(s) {
    // Add your logic.
    let regex1 = /[a-z]?[A-Z]?A$/gm
    let check = regex1.test(s)
    return check;
    return;
}
...
function ioEmail(email) {
    // Add your logic.
    let regex1 = /\bexample\b *@\bexample\b *.io$/gm
    let check = regex1.test(email)
    return check;
}
...
function imagesSearcher(text) {
    const result = text.match(/\b\w+\.(jpg|jpeg|png)\b/g);
